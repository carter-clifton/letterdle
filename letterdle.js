let rLet = [null, null, null, null, null];
...
for (let i = 0; i < 5; i++) {
    let digit = Math.floor(seededRandom(random_seed, 26)());
    if (digit === 27) {
        digit -= 1;
    }
    rLet[i] = letters[digit];
    random_seed *= 7547;
}

let answer = rLet.join("");
...
let valid_keys = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z"];
...
function enter_button() {
    if (wordLength === 5) {
        ...
        if (guessNumber === 6 && win === false) {
            ...
            document.getElementById("answer-if-fail").innerHTML = "The answer was: " + answer;
